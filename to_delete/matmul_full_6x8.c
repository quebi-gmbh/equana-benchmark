/**
 * Optimized Matrix Multiplication - 6x8 Micro-kernel Version
 *
 * 6x8 is a common choice in optimized BLAS implementations (like OpenBLAS for Haswell)
 * because it balances register usage with compute density.
 *
 * For f64x2: 6 rows × 8 cols = 48 elements = 24 f64x2 accumulators
 */
...
#ifdef USE_SIMD128
/**
 * 6x8 micro-kernel using WASM SIMD128
 * 8 columns × (6 rows / 2 per vector) = 24 accumulator vectors
 */
static void micro_kernel_6x8_simd128(int64_t kc, double alpha,
                                      const double *pa, const double *pb,
                                      double *C, int64_t ldc) {
    /* 8 columns, each with 3 f64x2 vectors (rows 0-1, 2-3, 4-5) */
    v128_t c00 = wasm_f64x2_splat(0.0), c01 = wasm_f64x2_splat(0.0), c02 = wasm_f64x2_splat(0.0);
    v128_t c10 = wasm_f64x2_splat(0.0), c11 = wasm_f64x2_splat(0.0), c12 = wasm_f64x2_splat(0.0);
    v128_t c20 = wasm_f64x2_splat(0.0), c21 = wasm_f64x2_splat(0.0), c22 = wasm_f64x2_splat(0.0);
    v128_t c30 = wasm_f64x2_splat(0.0), c31 = wasm_f64x2_splat(0.0), c32 = wasm_f64x2_splat(0.0);
    v128_t c40 = wasm_f64x2_splat(0.0), c41 = wasm_f64x2_splat(0.0), c42 = wasm_f64x2_splat(0.0);
    v128_t c50 = wasm_f64x2_splat(0.0), c51 = wasm_f64x2_splat(0.0), c52 = wasm_f64x2_splat(0.0);
    v128_t c60 = wasm_f64x2_splat(0.0), c61 = wasm_f64x2_splat(0.0), c62 = wasm_f64x2_splat(0.0);
    v128_t c70 = wasm_f64x2_splat(0.0), c71 = wasm_f64x2_splat(0.0), c72 = wasm_f64x2_splat(0.0);

    for (int64_t k = 0; k < kc; k++) {
        /* Load 6 rows of A as 3 vectors */
        v128_t a0 = wasm_v128_load(pa);      /* rows 0-1 */
        v128_t a1 = wasm_v128_load(pa + 2);  /* rows 2-3 */
        v128_t a2 = wasm_v128_load(pa + 4);  /* rows 4-5 */
    ...
    SCALE(c00); SCALE(c01); SCALE(c02);
    SCALE(c10); SCALE(c11); SCALE(c12);
    SCALE(c20); SCALE(c21); SCALE(c22);
    SCALE(c30); SCALE(c31); SCALE(c32);
    SCALE(c40); SCALE(c41); SCALE(c42);
    SCALE(c50); SCALE(c51); SCALE(c52);
    SCALE(c60); SCALE(c61); SCALE(c62);
    SCALE(c70); SCALE(c71); SCALE(c72);
    #undef SCALE

    /* Store results */
    #define STORE_COL(col, c0, c1, c2) do { \
        double *p = C + col * ldc; \
    ...
    STORE_COL(0, c00, c01, c02);
    STORE_COL(1, c10, c11, c12);
    STORE_COL(2, c20, c21, c22);
    STORE_COL(3, c30, c31, c32);
    STORE_COL(4, c40, c41, c42);
    STORE_COL(5, c50, c51, c52);
    STORE_COL(6, c60, c61, c62);
    STORE_COL(7, c70, c71, c72);
    #undef STORE_COL
}
#endif

static void micro_kernel_6x8_scalar(int64_t kc, double alpha,
                                     const double *pa, const double *pb,
                                     double *C, int64_t ldc) {
    double c[8][6] = {{0}};
...
static void macro_kernel(int64_t mc, int64_t nc, int64_t kc, double alpha,
    ...
    for (j = 0; j + NR <= nc; j += NR) {
        for (i = 0; i + MR <= mc; i += MR) {
            #ifdef USE_SIMD128
            micro_kernel_6x8_simd128(kc, alpha,
                                      pa + i * kc,
                                      pb + j * kc,
                                      C + i + j * ldc, ldc);
            #else
            micro_kernel_6x8_scalar(kc, alpha,
                                     pa + i * kc,
                                     pb + j * kc,
                                     C + i + j * ldc, ldc);
            #endif
...
__attribute__((export_name("dgemm_6x8")))
void dgemm_6x8(int32_t m, int32_t n, int32_t k,
...
__attribute__((export_name("matmul_f64_6x8")))
void matmul_f64_6x8(int32_t M, int32_t N, int32_t K,
